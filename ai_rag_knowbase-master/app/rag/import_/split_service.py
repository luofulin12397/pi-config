...
def load_markdown_content(state: ImportGraphState) -> tuple[str, str,Path,list,str]:
    """
    ...
    return md_content, file_title , Path(md_path),allowed_roles,imported_by

def _line_range_for_substring(
    md_content: str,
    ...
    """在父 chunk 行区间内定位子串，返回 (start_line, end_line, next_search_offset)。"""
    parent_start = parent_start or 1
    parent_end = parent_end or parent_start
    lines = md_content.split("\n")
    slice_lines = lines[parent_start - 1:parent_end]
    slice_text = "\n".join(slice_lines)
    needle = sub_text.strip()
    if not needle:
        return parent_start, parent_end, search_offset

    idx = slice_text.find(needle, search_offset)
    if idx < 0:
        return parent_start, parent_end, search_offset

    line_start = parent_start + slice_text[:idx].count("\n")
    line_end = parent_start + slice_text[: idx + len(needle)].count("\n")
    next_offset = idx + len(needle)
    return line_start, max(line_end, line_start), next_offset
...
def split_by_titles(md_content: str, file_title: str,allowed_roles: list,imported_by:str ) -> list[dict]:
    """
    ...
    :param allowed_roles: 允许访问角色
    :param imported_by: 导入人员
    ...
    # 逐行遍历 MD 内容（行号与原始 MD 文件一致，1 起始）
    for line_no, raw_line in enumerate(lines, start=1):
        line = raw_line.strip()

        # 空行跳过（不计入 chunk 正文，但行号仍连续）
        if not line:
            continue
...
def _split_long_section(
    ...
    sub_sections = []
    parent_start = section.get("start_line")
    parent_end = section.get("end_line")
    search_offset = 0
    #4. 遍历拆分后的正文片段，生成子块
    for index, chunk_text in enumerate(splitter.split_text(body), start=1):
        ...
        start_line, end_line, search_offset = _line_range_for_substring(
            md_content, parent_start, parent_end, text, search_offset
        )
        ...
        sub_sections.append({
            ...
            "imported_by": section.get("imported_by"),
        })
    ...
    for next_chunk in final_chunks:
        ...
        if is_lt_chunk_size and is_same_parent_title:
            ...
            if len(merged_content) <= max_length:
                ...
                s1 = start_chunk.get("start_line")
                s2 = next_chunk.get("start_line")
                e1 = start_chunk.get("end_line")
                e2 = next_chunk.get("end_line")
                if s1 is not None and s2 is not None:
                    start_chunk["start_line"] = min(s1, s2)
                if e1 is not None and e2 is not None:
                    start_chunk["end_line"] = max(e1, e2)
...
def refine_chunks(
    ...
    for chunk in chunks:
        if len(chunk['content']) > max_len:
            # 拆分过长的块，并加入结果列表
            final_chunks.extend(_split_long_section(chunk, md_content, max_len))
        else:
...
def split_document(state: ImportGraphState) -> ImportGraphState:
    ...
    md_content, file_title , md_path_obj,allowed_roles,imported_by = load_markdown_content(state)
    # 2. 按 Markdown 标题（#、##、###）进行【智能语义切块】（保持段落完整性）
    chunks = split_by_titles(md_content, file_title, allowed_roles,imported_by)
    # 3. 精细切割(涉及长切和短切处理,返回最终处理的chunks)
    final_chunks = refine_chunks(chunks, md_content)
    # 4. 备份final_chunks内容
