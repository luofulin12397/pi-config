...
class HistoryMongoTool:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.user_context = self.db["user_context"]
            self.user_context.create_index([("user_id", 1)], unique=True)
            self.query_cache = self.db["query_cache"]
            self.query_cache.create_index([("cache_key", 1)], unique=True)
            self.import_tasks = self.db["import_tasks"]
            self.import_tasks.create_index([("task_id", 1)], unique=True)
            self.import_tasks.create_index([("created_at", -1)])
            self.session_context = self.db["session_context"]
            self.session_context.create_index([("session_id", 1)], unique=True)
...
def clear_history_by_user(user_id: str) -> int:
    """清空指定用户的所有历史对话记录。"""
    mongo_tool = get_history_mongo_tool()
    try:
        result = mongo_tool.chat_message.delete_many({"user_id": user_id})
...
def clear_history(session_id: str) -> int:
    ...
    try:
        ...
        mongo_tool.session_context.delete_many({"session_id": session_id})
...
def get_recent_messages_by_user(user_id: str, limit: int = 10) -> list[dict[str, Any]]:
    """查询指定用户的最近 N 条对话记录（跨会话）。"""
    mongo_tool = get_history_mongo_tool()
    try:
...
def get_top_frequent_questions_by_user(user_id: str, limit: int = 10) -> list[dict[str, Any]]:
    """统计用户历史提问频次，返回 Top N 问题（按 count 降序）。"""
    mongo_tool = get_history_mongo_tool()
    try:
        pipeline = [
        ...
        logger.error(f"Error getting frequent questions for user {user_id}: {e}")
        return []
...
def get_all_messages_by_user(user_id: str) -> list[dict[str, Any]]:
    """获取用户全部历史，按时间正序。"""
    mongo_tool = get_history_mongo_tool()
    try:
        cursor = mongo_tool.chat_message.find({"user_id": user_id}).sort("ts", 1)
        messages = list(cursor)
        for msg in messages:
            if "_id" in msg:
                msg["_id"] = str(msg["_id"])
...
def get_all_messages(session_id: str) -> list[dict[str, Any]]:
    ...
    mongo_tool = get_history_mongo_tool()
    try:
        cursor = mongo_tool.chat_message.find({"session_id": session_id}).sort("ts", 1)
        messages = list(cursor)
        for msg in messages:
            if "_id" in msg:
                msg["_id"] = str(msg["_id"])
        ...
        logger.error(f"Error getting all messages for session {session_id}: {e}")
        return []
...
def get_session_context(session_id: str) -> dict[str, Any] | None:
    ...
    读取会话压缩摘要缓存
    :param session_id: 会话 ID
    :return: session_context 文档或 None
    """
    mongo_tool = get_history_mongo_tool()
    try:
        doc = mongo_tool.session_context.find_one({"session_id": session_id})
...
def upsert_session_context(session_id: str, data: dict[str, Any]) -> bool:
    """
    写入或更新会话压缩摘要
    :param session_id: 会话 ID
    :param data: 摘要数据
    ...
    try:
        ...
        mongo_tool.session_context.update_one(
            {"session_id": session_id},
            {"$set": payload},
            upsert=True,
        )
        ...
        logger.error(f"Error upserting session context for {session_id}: {e}")
        return False
...
def get_user_context(user_id: str) -> dict[str, Any] | None:
    mongo_tool = get_history_mongo_tool()
    try:
        doc = mongo_tool.user_context.find_one({"user_id": user_id})
        if doc and "_id" in doc:
            doc["_id"] = str(doc["_id"])
        return doc
    except Exception as e:
...
def upsert_user_context(user_id: str, data: dict[str, Any]) -> bool:
    mongo_tool = get_history_mongo_tool()
    try:
        payload = {"user_id": user_id, **data}
        mongo_tool.user_context.update_one({"user_id": user_id}, {"$set": payload}, upsert=True)
        return True
    except Exception as e:
        logger.error(f"Error upserting user context for {user_id}: {e}")
        return False


def get_query_cache_entry(cache_key: str) -> dict[str, Any] | None:
    mongo_tool = get_history_mongo_tool()
    try:
        return mongo_tool.query_cache.find_one({"cache_key": cache_key})
    except Exception as e:
        logger.error(f"Error getting query cache {cache_key}: {e}")
        return None


def upsert_query_cache_entry(cache_key: str, data: dict[str, Any]) -> bool:
    mongo_tool = get_history_mongo_tool()
    try:
        payload = {"cache_key": cache_key, **data, "updated_at": datetime.now().timestamp()}
        mongo_tool.query_cache.update_one({"cache_key": cache_key}, {"$set": payload}, upsert=True)
        return True
    except Exception as e:
        logger.error(f"Error upserting query cache {cache_key}: {e}")
        return False


def save_import_task(task: dict[str, Any]) -> bool:
    mongo_tool = get_history_mongo_tool()
    try:
        ...
        logger.error(f"Error saving import task: {e}")
        return False
...
def get_import_task(task_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.error(f"Error getting import task {task_id}: {e}")
        return None


def list_import_tasks(limit: int = 50, imported_by: str | None = None) -> list[dict[str, Any]]:
    mongo_tool = get_history_mongo_tool()
    try:
        query = {"imported_by": imported_by} if imported_by else {}
        cursor = mongo_tool.import_tasks.find(query).sort("created_at", -1).limit(limit)
        rows = list(cursor)
        for row in rows:
            if "_id" in row:
                row["_id"] = str(row["_id"])
        ...
        logger.error(f"Error listing import tasks: {e}")
        return []
...
def delete_import_task(task_id: str) -> int:
    mongo_tool = get_history_mongo_tool()
    try:
        result = mongo_tool.import_tasks.delete_one({"task_id": task_id})
        return result.deleted_count
    except Exception as e:
        logger.error(f"Error deleting import task {task_id}: {e}")
        return 0
