...
def get_history_messages(user_id: str, limit: int = 10) -> list[dict]:
    """
    获取历史聊天记录! 倒序 limit=10
      只获取有效的聊天记录! item_names有数据为判断依据
    :param user_id: 用户 ID（跨会话）
    :param limit: 筛选数量
    :return: 有效数据集合
    """
    history_message_list = history_repository.list_recent_by_user(user_id=user_id, limit=limit)
...
def query_item_name_milvus(item_names:list[str])->dict[str,list[dict]]:
    ...
    for item_name in item_names:
        ...
        # 6. 单条结果解析
        if not milvus_search_result:
            logger.warning(
                f"模型提供的: {item_name} Milvus 检索失败或无结果，跳过本次"
            )
            continue
        real_result = milvus_search_result[0] if len(milvus_search_result) > 0 else []
        if not real_result:
        ...
        current_item_name_list = [
            {
                "item_name": item_dict.get("entity", {}).get("item_name"),
                "score": item_dict.get("distance", 0),
                "allowed_roles": item_dict.get("entity", {}).get("allowed_roles")
                or item_dict.get("allowed_roles")
                or [],
            }
            for item_dict in real_result
        ]
...
def select_item_names(milvus_result_dict):
    ...
    for item_name , milvus_list in milvus_result_dict.items():
        ...
        high_item_names = [ {item['item_name']:item['allowed_roles']} for item in milvus_list if item['score'] > 0.7 ]
        md_item_names =   [ {item['item_name']:item['allowed_roles']} for item in milvus_list if 0.6 < item['score'] <= 0.7]
...
def change_state_status(state, item_name_dict, rewritten_query):
    ...
    if confirmed_item_name_list and len(confirmed_item_name_list) > 0:
        # 有确认的!
        state['item_names_roles_dict'] = confirmed_item_name_list
...
def confirm_item_name(state: QueryGraphState) -> QueryGraphState:
    ...
    # 2. 优先使用历史压缩节点产出的上下文，否则回退到近10条原文
    history_text = state.get("history_context")
    if not history_text:
        history_message_list: list[dict] = get_history_messages(state.get("user_id", ""), limit=10)
