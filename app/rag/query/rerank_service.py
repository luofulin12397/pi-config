...
def get_rewritten_query_and_validate(state):
    ...
    # 非空判定：向量检索或网络搜索至少有一路有结果
    if not rewritten_query:
        logger.error("rewritten_query 为空,业务无法继续进行!!")
        raise ValueError("rewritten_query 为空,业务无法继续进行!!")
    if len(rrf_chunks) == 0 and len(web_search_docs) == 0:
        logger.error("rrf_chunks 与 web_search_docs 均为空,业务无法继续进行!!")
        raise ValueError("rrf_chunks 与 web_search_docs 均为空,业务无法继续进行!!")
