...
def resolve_input_file(state: ImportGraphState) -> ImportGraphState:
    ...
    if not allowed_roles or not imported_by:
        logger.error(f"allowed_roles or imported_by is None, allowed_roles:{allowed_roles}, imported_by:{imported_by}")
        raise ValueError("allowed_roles or imported_by is None")
    # 7. 返回state 处理完毕
    return state